...
export type ViewMode = 'app' | 'flow-overview';

export default function App() {
  const [isGenerating, setIsGenerating] = useState(true);
  const [showVoiceListening, setShowVoiceListening] = useState(false);
  const [currentMode, setCurrentMode] = useState<RemoteMode>('tv');
  const [pendingMode, setPendingMode] = useState<RemoteMode | null>(null);
  ...
  const handleModeChangeFromMenu = (mode: RemoteMode) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-900 via-slate-950 to-black text-white overflow-hidden">
      {/* Main Interface - Full Viewport */}
      <motion.div 
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        ...
        <AnimatePresence mode="wait">
          {isGenerating ? (
            ...
            <AdaptiveRemote 
              ...
              onModeChange={handleModeChangeFromMenu}
            />
          )}
        </AnimatePresence>
